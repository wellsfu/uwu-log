...
技能名稱(致命一擊)、是否標記「一擊必殺」、死亡時間點(pull 內相對秒數)、受到的傷害、受到的治療、DOT 持續秒數(若有)。同時記錄死亡前後約 10~15 秒內的其他事件(誰施放了什麼、誰吃了什麼 buff/debuff、附近有沒有其他人死亡)——Step 5 要用。
...
## Step 5 — 往前往後查事件,別預設是死者自己的錯

機制表的死因判讀文字通常是寫給「這個技能」的通用說法,容易讓人直接套到死者本人身上。但很多死亡的根本原因其實是別人造成的,不是死者站錯位置。查 Step 2 記錄的死亡前後事件,排除以下這些情況:

- **分攤/集合類機制**:判讀是「分攤人數不足」時,查是誰沒有及時歸位集合,不是只寫死者自己
- **減傷/無敵沒開好**:機制設計上該由坦克/特定人開減傷或無敵,結果沒開,導致波及的隊友死亡——根本原因是沒開技能的那個人
...
- 看到死因判讀寫「站位錯誤」就直接寫成死者自己的錯,沒查 Step 5 的前後事件確認是否為隊友沒開減傷/沒歸位、或前一次死亡連鎖造成
